	...
	if (empty($respuesta['mensaje'])) //Consultar si existe alguna validación
	{
		$selectComprobante = 
			"SELECT
				...
				cli.direccion,
				cli.num_documento,
				par2.descripcion AS des_tipo_documento,
				CONCAT_WS(' ', cli.nombre_razon_social, cli.seg_nombre, cli.pri_apellido, cli.seg_apellido) AS nombre_razon_social,
				com.tip_comprobante AS tip_comprobante,
				par1.descripcion AS des_comprobante,
				GROUP_CONCAT(DISTINCT par3.descripcion ORDER BY par3.descripcion SEPARATOR ', ') AS desc_medio_pago,
				com.condicion_pago,
				com.desc_medio_pago as desc_condicion_pago,
				com.guia_remision,
				DATE_FORMAT(com.fec_emision, '%d-%m-%Y %H:%i:%s') AS fec_emision,
				DATE_FORMAT(com.fec_vencimiento, '%d-%m-%Y %H:%i:%s') AS fec_ven,
				com.ord_compra,
				IF(com.observaciones = '', '-', com.observaciones) AS observaciones,
				cli.correo,
				com.mon_neto,
				com.mon_igv,
				com.mon_total,
				com.aplica_detraccion,
				com.tip_detraccion,
				com.por_detraccion,
				com.mon_detraccion,
				com.cod_medio_pago_detraccion,
				com.usu_creacion,
				IF(cuo.fecha IS NULL, DATE_FORMAT(com.fec_vencimiento, '%d-%m-%Y %H:%i:%s'), DATE_FORMAT(cuo.fecha, '%d-%m-%Y %H:%i:%s')) AS fec_vencimiento
			FROM
				comprobantes com
			JOIN
				clientes cli ON cli.id = com.id_cliente
			LEFT JOIN
				pagos pag ON pag.id_comprobante = com.id
			JOIN
				parametros par1 ON par1.codigo = com.tip_comprobante AND par1.padre = 8
			JOIN
				parametros par2 ON par2.codigo = cli.tip_documento AND par2.padre = 12
			LEFT JOIN
				parametros par3 ON par3.codigo = pag.codigo_medio_pago AND par3.padre = 4
			LEFT JOIN
				cuotas cuo ON cuo.id_comprobante = com.id
			WHERE
				com.id IN ('$idComprobante')
			GROUP BY
				com.id, com.id_cliente, cli.tip_documento, cli.direccion, cli.num_documento, par2.descripcion,
				nombre_razon_social, com.tip_comprobante, par1.descripcion, com.condicion_pago,
				com.desc_medio_pago, com.guia_remision, com.fec_emision, com.fec_vencimiento, com.ord_compra,
				com.observaciones, cli.correo, com.mon_neto, com.mon_igv, com.mon_total, com.aplica_detraccion,
				com.tip_detraccion, com.por_detraccion, com.mon_detraccion, com.cod_medio_pago_detraccion, com.usu_creacion
			ORDER BY
				cuo.fecha DESC";
		...
		$selectCuotas = "
		    SELECT DATE_FORMAT(cuo.fecha, '%d-%m-%Y') as fecha_cuota, cuo.monto 
		    FROM cuotas cuo 
		...
		$selectDetComprobante = "
		    SELECT dcom.id_producto AS id_producto, pro.codigo AS cod_producto, pro.descripcion AS des_producto, dcom.cantidad, 
				dcom.precio AS precio_unitario, dcom.precio*dcom.cantidad_final AS precio_total, pro.unidad_medida, 
		...
		//$ruta = "https://www.nubefact.com/api/v1/c50fb9e7-1b7f-48de-8398-219df1a6b417";
		$ruta = "https://api.nubefact.com/api/v1/85f4c219-d242-41c7-81b5-d4b99ea44242";

		//TOKEN para enviar documentos
		//$token = "2c19da69d71a44e6b9c0de0db6acdaf459357dcdcd954fe4abf52f0443709dc2";
		...
		if (mysqli_num_rows($resultSelectComprobante) > 0) 
		{
			while ($rowSelectComprobante = mysqli_fetch_assoc($resultSelectComprobante)) 
			{
				...
				if(mysqli_num_rows($resultSelectDetComprobante) > 0) 
				{
					...
					{
						$cantidad	    = number_format($rowSelectDetComprobante['cantidad_final'], 2, '.', '');
						$precioUnitarioConIGV = number_format($rowSelectDetComprobante['precio_unitario'], 2, '.', '');
						$precioUnitarioSinIGV = number_format($precioUnitarioConIGV/1.18, 2, '.', '');
						...
						{
							$stockProducto = $rowSelectStockProducto['stock'] - $cantidad;
						}
						...
						$desVenta[] = array(
							...
							"cantidad"                  => $cantidad,
							"valor_unitario"            => $precioUnitarioSinIGV,
							"precio_unitario"           => $precioUnitarioConIGV,
							"descuento"                 => "",
							"subtotal"                  => $cantidad*$precioUnitarioSinIGV,
							"tipo_de_igv"               => "1",
							"igv"                       => $cantidad*$precioUnitarioConIGV - $cantidad*$precioUnitarioSinIGV,
							"total"                     => $cantidad*$precioUnitarioConIGV,
				...
				$data = array(
					...
					"detraccion_tipo"                   => $rowSelectComprobante['tip_detraccion'] == "0" ? "" : $rowSelectComprobante['tip_detraccion'], 
					"detraccion_total"                  => $rowSelectComprobante['mon_detraccion'] == 0 ? "" : $rowSelectComprobante['mon_detraccion'], 
					"detraccion_porcentaje"             => $rowSelectComprobante['por_detraccion'] == "0" ? "" : $rowSelectComprobante['por_detraccion'], 
					"medio_de_pago_detraccion"          => $rowSelectComprobante['cod_medio_pago_detraccion'] == "0" ? "" : $rowSelectComprobante['cod_medio_pago_detraccion'],
					...
					"codigo_unico"                      => "", 
					"condiciones_de_pago"               => $condicionesPago, 
					"medio_de_pago"                     => $condicionesPago, 
					"placa_vehiculo"                    => "", 
			...
			else 
			{
				...
				$updateDatosComprobante = "
					...
					SET com.ser_comprobante = '$serieComprobante', com.num_comprobante = '$numeroComprobante', 
						com.emitido = 1, com.condicion_pago = '$condicionPago', com.url = '$enlacePDF', 
