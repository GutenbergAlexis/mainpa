...
<head>
    ...
    <title>Buscar ventas - Mainpasoft</title>
    ...
    <div id="wrapper">
                    ...
                    <ul class="nav" id="side-menu">
                        <li class="active">
                            ...
                            <ul class="nav nav-second-level">
                                ...
                                <li>
                                    <a class="active" href="#">Buscar ventas</a>
                                </li>
                                <li>
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">Buscar ventas</h1>
					</div>
<?php
...
?>
					<div class="panel-body">
						<div class="col-lg-3 col-mb-6 col-sm-12">
							...
							<input class="form-control" type="text" id="codigo-venta" name="codigo-venta" placeholder="Código de venta" />
						</div>
						<div class="col-lg-3 col-mb-6 col-sm-12">
							<label for="numero-documento">Número de DNI/RUC</label>
							<input class="form-control" type="text" id="numero-documento" name="numero-documento" placeholder="Número de DNI/RUC" />
						</div>
						<div class="col-lg-3 col-mb-6 col-sm-12">
						...
						<div class="col-lg-3 col-mb-6 col-sm-12">
							<label for="numero-comprobante">Número de comprobante</label>
							<input class="form-control" type="text" id="numero-comprobante" name="numero-comprobante" placeholder="Número de comprobante" />
						</div>
					</div>
					<div class="panel-body">
					<?php 
					...
					?>
						<div class="col-lg-12 col-mb-12 col-sm-12">
							<button type="button" class="btn btn-default" data-toggle="modal" onclick="buscarVentas()" title="buscar"><i class="fa fa-search"></i> Buscar</button>
						</div>
					</div>
					<div class="panel-body">
						<div class="col-lg-12 col-mb-12 col-sm-12">
							<h4>Resumen de ventas</h4>
							<div id="ventas" class="centrar"></div>
						</div>
					<?php
						} 
						else 
					...
					?>
						<div class="col-lg-12 col-mb-12 col-sm-12">
							<button type="button" class="btn btn-default" data-toggle="modal" onclick="leerComprobantes()" title="buscar"><i class="fa fa-search"></i> Buscar</button>
						</div>
					</div>
					<div class="panel-body">
						<div class="col-lg-12 col-mb-12 col-sm-12">
							<h4>Resumen de ventas</h4>
							<div id="comprobantes" class="centrar"></div>
						</div>
					<?php
						}
					?>
...
?>
	...
	<div class="modal fade" id="mod-cargar-comprobante" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	</div>
	
	<!-- MODAL ACTUALIZAR CUOTA -->
	<div class="modal fade" id="mod-actualizar-cuota" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	</div>
...
<script>
// Función para buscar ventas con paginación
function buscarVentas(pagina = 1) {
    var codVenta = $('#codigo-venta').val();
    var numDocumento = $('#numero-documento').val();
    var fechaDesde = $('#fecha-desde').val();
    var fechaHasta = $('#fecha-hasta').val();
    var numComprobante = $('#numero-comprobante').val();
    
    $.ajax({
        url: '../../ajax/ingresos/buscarVentas.php',
        type: 'POST',
        data: {
            codVenta: codVenta,
        ...
        success: function(response) {
            $('#ventas').html(response);
        },
        error: function() {
            $('#ventas').html('<p>Error al cargar las ventas.</p>');
        }
    });
}

// Función para leer comprobantes con paginación
function leerComprobantes(pagina = 1) {
    var codVenta = $('#codigo-venta').val();
    var numDocumento = $('#numero-documento').val();
    var fechaDesde = $('#fecha-desde').val();
    var fechaHasta = $('#fecha-hasta').val();
    ...
    $.ajax({
        url: '../../ajax/ingresos/leerComprobantes.php',
        type: 'POST',
        data: {
            codVenta: codVenta,
            numDocumento: numDocumento,
        ...
        error: function() {
            $('#comprobantes').html('<p>Error al cargar los comprobantes.</p>');
        }
    });
}
...
$(document).on('click', '.pagination a', function(e) {
    ...
    // Determinar qué función llamar según el perfil
    var perfil = <?php echo $_SESSION['perfil']; ?>;
    if (perfil == 1 || perfil == 3) {
        buscarVentas(pagina);
    } else {
        leerComprobantes(pagina);
    }
});

// Cargar datos al iniciar (opcional)
$(document).ready(function() {
    // Si quieres que cargue datos automáticamente al abrir la página
    var perfil = <?php echo $_SESSION['perfil']; ?>;
    if (perfil == 1 || perfil == 3) {
        buscarVentas(1);
    } else {
