...
if(mysqli_num_rows($resultSelectCotizacion) > 0) 
{
	while ($rowSelectCotizacion = mysqli_fetch_assoc($resultSelectCotizacion)) 
	{
		...
		$pdf->Cell(180, 6, utf8_decode('BBVA: 0011-0832-01-0001644701'), 0, 1, '');
		$pdf->Cell(180, 6, utf8_decode('BCP: 191-2529382-0-12'), 0, 1, '');
		$pdf->Cell(180, 6, utf8_decode('CCI BCP: 00219100252938201259'), 0, 1, '');
		$pdf->Cell(180, 6, utf8_decode('CUENTA DETRACCIÓN'), 0, 1, '');
		$pdf->Cell(180, 6, utf8_decode('BANCO DE LA NACIÓN: 00-062-101334'), 0, 1, '');
		$pdf->Ln();
		$pdf->SetFont('Arial', 'B', 10);
		$pdf->Cell(180, 6, utf8_decode('P & D INVERSIONES E IMPORTACIONES S.A.C.'), 0, 1, '');
		$pdf->Cell(180, 6, utf8_decode('RUC 20603369000'), 0, 1, '');
	}
}
